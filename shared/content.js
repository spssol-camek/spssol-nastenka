...
export function validateContent(value) {
  ...
  const posts = value.posts.map(p => {
    ...
    if (p.markdown !== undefined && typeof p.markdown !== 'boolean') throw new Error('Neplatná volba formátování.');
    postIds.add(p.id);
    return { id: p.id, title: p.title.trim(), description: p.description, ...(p.url ? { url: p.url } : {}), ...(p.image ? { image: p.image } : {}), ...(p.type === 'links' ? {links:p.links.map(l => ({title:l.title.trim(),url:l.url.trim()}))} : {}), ...(p.expanded === true ? {expanded:true} : {}), ...(p.markdown === true ? {markdown:true} : {}), type: p.type, tags: [...new Set(p.tags)], boards: [...new Set(p.boards)], size: p.size, published: p.published, publishedAt: p.publishedAt, createdAt: p.createdAt, ...(p.updatedAt ? { updatedAt: p.updatedAt } : {}) };
  });
  return { boards, posts };
}
